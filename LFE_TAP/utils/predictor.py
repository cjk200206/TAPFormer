...
from LFE_TAP.utils.model_utils import get_points_on_a_grid, get_sift_sampled_pts, get_uniformly_sampled_pts, smart_cat
...
class CoTrackerPredictor(torch.nn.Module):
    ...
    def _compute_sparse_tracks(
        ...
        # events = F.interpolate(events, tuple(self.interp_shape), mode="bilinear", align_corners=True)
        # events = events.reshape(B, T, C_e, self.interp_shape[0], self.interp_shape[1])
        ...
        if queries is not None:
            ...
            grid_pts = get_points_on_a_grid(
                grid_size, self.interp_shape, device=queries.device
            )
        ...
        tracks, visibilities, __ = self.model.forward(
            rgbs=video, events=events, queries=queries, iters=6
        )
